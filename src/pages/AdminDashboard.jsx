...
const AdminDashboard = ({ onLogout }) => {
  ...
  const fetchStudents = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('cp2_students').select('*').order('student_name');
  ...
  const deleteStudent = async (id, name) => {
    ...
    const { error } = await supabase.from('cp2_students').delete().eq('id', id);
  ...
  const resetProgress = async (id, name) => {
    ...
    const { error } = await supabase.from('cp2_students').update({ progress: {} }).eq('id', id);
        ...
        <div className="bg-slate-800/50 rounded-2xl border border-slate-700 overflow-hidden">
          <div className="p-6 border-b border-slate-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={fetchStudents} className="px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-lg">Refresh</button>
            </div>
          </div>
